...
const TargetCursor = ({
  ...
  useEffect(() => {
    ...
    const moveHandler = (e) => {
      ...
      if (!isMouseInContainer(x, y) || !target || !containerRef.current.contains(target)) {
    ...
    const enterHandler = (e) => {
      ...
      if (!containerRef.current.contains(directTarget)) return;
      ...
      const updateCorners = (mouseX, mouseY) => {
        if (!isMouseInContainer(mouseX, mouseY)) return;
        ...
        const [tlc, trc, brc, blc] = Array.from(cornersRef.current); // Fixed: Define variables here
  ...
  }, [targetSelector, spinDuration, moveCursor, constants, hideDefaultCursor, containerRef]);
